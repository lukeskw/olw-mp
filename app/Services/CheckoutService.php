...
class CheckoutService {
    ...
    public function creditCardPayment($data, $user, $address): Payment|PaymentException {
        $payment = new Payment();
...
//        );
        ...
        throw_if(!$payment->id || $payment->status === 'rejected', PaymentException::class,
            $payment?->error?->message ?? "Verifique os dados do cartão");

        return $payment;
    }

    public function pixOrBankSlipPayment($data, $user, $address): Payment|PaymentException
    {
        $payment = new Payment();
